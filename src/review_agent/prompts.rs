//! System prompt and ReviewInput → user message text for the review ReAct agent.
//!
...
RULES:
1. Use get_pr_context(part: "title" | "description" | "diff" | "files") to re-read a specific part if needed.
...
/// `get_pr_context` (title, description, diff, files). Used by both the initial
/// user message and by `ReviewToolSource::get_pr_context`.
